...
public class LocatarioView
{
    ...
    private void AlugarLivro()
    {
        ...
        int idLivro = Utils.LerInteiro("ID do Livro: ");
        Livro livro = _livroController.BuscarLivroPorId(idLivro);

        if (livro == null || !livro.Disponivel)
        {
            Utils.ExibirMensagemErro("Livro não disponível.");
            return;
        }

        int idLocatario = Utils.LerInteiro("ID do Locatário: ");
        Pessoa locatario = _pessoaController.BuscarPessoaPorId(idLocatario);

        if (locatario == null || locatario.Tipo != TipoPessoa.Locatario)
        {
            Utils.ExibirMensagemErro("Locatário não encontrado ou não autorizado.");
            return;
        }
        ...
        Utils.ExibirMensagem("Livro alugado com sucesso.");
    }
